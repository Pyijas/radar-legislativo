...
Coleta, por padrão, só PLs federais do Brasil (Câmara dos Deputados +
Senado Federal) — Congresso dos EUA e Congresso do Chile continuam
...
impacto em saúde/farma em duas camadas:
  ...
  1. Heurística por palavras-chave (PT ou EN) — sempre roda, 100% gratuita,
     determinística.
  2. IA (Anthropic, Gemini ou Ollama local) — roda por cima quando há
     provedor configurado e sobrescreve a heurística com uma leitura mais
     precisa.

Uso:
    python main.py                      # últimos 7 dias, Câmara + Senado (Brasil)
    python main.py --dias 15
    python main.py --fontes camara,senado,eua,chile  # inclui EUA/Chile nesta execução
    ...
    python main.py --sem-ia             # força heurística mesmo com provedor de IA configurado (backfill)
    python main.py --sem-monitoramento  # pula notícias/agenda, só PLs
"""
...
_FONTES_PADRAO = ["camara", "senado"]  # só Brasil por padrão — EUA/Chile disponíveis via --fontes
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--fontes", type=str, default=",".join(_FONTES_PADRAO),
                   help="fontes de PLs a coletar, separadas por vírgula: camara, senado, eua, chile "
                        "(padrão: camara,senado — só Brasil)")
    ...
    p.add_argument("--sem-ia", action="store_true",
                   help="força classificação só por heurística, mesmo com provedor de IA configurado "
                        "(use isso pra backfill de histórico grande, sem gastar cota de IA)")
    p.add_argument("--sem-monitoramento", action="store_true",
                   help="pula a coleta de notícias (Anvisa/MS/ANS) e da agenda do Presidente, "
                        "roda só a parte de PLs")
    p.add_argument("--dias-monitoramento", type=int, default=14,
                   help="janela de dias pra notícias/agenda (padrão: 14 — os sitemaps de notícia "
                        "não guardam histórico muito além disso, ver docstring dos clients)")
    return p.parse_args()
...
def _coletar_chile(dias: int, tipos: list[str]) -> list[dict]:
    with console.status("Buscando projetos na tramitação do Congresso do Chile..."):
        try:
            itens = chile_client.listar_novas(dias)
        except Exception as exc:
            console.print(f"[red]Chile: erro ao consultar o serviço de tramitação:[/red] {exc}")
            return []

    novos = [it for it in itens if not storage.ja_processada("CL", it["casa"], it["id_externo"])]
    console.print(f"Chile: [bold]{len(itens)}[/bold] projetos encontrados no período, "
                  f"[bold]{len(novos)}[/bold] ainda não processados.")

    registros = []
    for it in novos:
        try:
            texto = pdf_extract.extrair_texto(it.get("url_inteiro_teor"))
            registros.append({**it, "pais": "CL", "idioma": "es", "texto_inteiro_teor": texto})
        except Exception as exc:
            console.print(f"[red]Chile: erro ao processar boletim {it.get('id_externo')}:[/red] {exc}")
    return registros
...
def coletar_e_classificar(dias: int, tipos: list[str], tem_ia: bool, fontes: list[str]) -> None:
    for fonte in fontes:
        ...
        if not coletor:
            console.print(f"[red]Fonte desconhecida: '{fonte}' (use camara, senado, eua ou chile)[/red]")
            continue
...
def _coletar_e_classificar_noticias(dias: int, tem_ia: bool) -> None:
    ...
    for nome, (cliente, idioma, modo) in _FONTES_NOTICIAS.items():
        try:
            ...
            console.print(f"[red]Notícias {nome}: erro ao consultar:[/red] {exc}")
            continue

        novos = [it for it in itens if not storage.ja_processada_noticia(it["fonte"], it["id_externo"])]
        console.print(f"Notícias {nome}: [bold]{len(itens)}[/bold] encontradas no período, "
        ...
        for it in novos:
            ...
            it.update(
                ...
                areas_impactadas=c["areas_impactadas"], tipo_impacto=c["tipo_impacto"],
                nivel_impacto=c["nivel_impacto"],
            )
            if tem_ia:
                try:
                    ...
                    it.update(
                        ...
                        justificativa_relevancia=c_ia["justificativa_relevancia"],
                        resumo_ia=c_ia["resumo"], areas_impactadas=c_ia["areas_impactadas"],
                        tipo_impacto=c_ia["tipo_impacto"], nivel_impacto=c_ia["nivel_impacto"],
                    )
                except ClassificacaoIndisponivel as exc:
                    console.print(f"[yellow]Aviso:[/yellow] falha ao classificar notícia "
                                  f"{it.get('id_externo')} por IA (mantendo heurística): {exc}")
            storage.salvar_noticia(it)
...
def _coletar_e_classificar_agenda(dias: int, tem_ia: bool) -> None:
    ...
    for nome, cliente in _FONTES_AGENDA.items():
        try:
            ...
            console.print(f"[red]Agenda {nome}: erro ao consultar:[/red] {exc}")
            continue

        novos = [ev for ev in eventos if not storage.ja_processado_evento(ev["autoridade"], ev["id_externo"])]
        console.print(f"Agenda {nome}: [bold]{len(eventos)}[/bold] compromissos no período, "
                      f"[bold]{len(novos)}[/bold] ainda não processados.")

        for ev in novos:
            c = heuristic_classify.classificar(ev.get("titulo") or "", ev.get("local"), idioma="pt")
            ev.update(
                ia_disponivel=0, fonte_classificacao="heuristica", relevante=c["relevante"],
                justificativa_relevancia=c["justificativa_relevancia"], resumo_ia=c["resumo"],
                areas_impactadas=c["areas_impactadas"], tipo_impacto=c["tipo_impacto"],
            ...
            if tem_ia:
                try:
                    c_ia = classificar_evento_agenda(ev.get("titulo") or "", ev.get("autoridade") or "", ev.get("local"))
                    ev.update(
                        ia_disponivel=1, fonte_classificacao="ia", relevante=c_ia["relevante"],
                        justificativa_relevancia=c_ia["justificativa_relevancia"],
                        resumo_ia=c_ia["resumo"], areas_impactadas=c_ia["areas_impactadas"],
                    ...
                    console.print(f"[yellow]Aviso:[/yellow] falha ao classificar evento de agenda "
                                  f"{ev.get('id_externo')} por IA (mantendo heurística): {exc}")
            storage.salvar_evento_agenda(ev)
...
def montar_relatorio_noticias(mostrar_todas: bool) -> list:
    linhas = storage.listar_noticias_recentes()
    if not mostrar_todas:
        linhas = [l for l in linhas if l["relevante"] in (1, None)]
    return sorted(linhas, key=lambda l: _NIVEL_RANK.get(l["nivel_impacto"], 3))


def montar_relatorio_agenda(mostrar_todas: bool) -> list:
    linhas = storage.listar_eventos_recentes()
    if not mostrar_todas:
        linhas = [l for l in linhas if l["relevante"] in (1, None)]
    return sorted(linhas, key=lambda l: _NIVEL_RANK.get(l["nivel_impacto"], 3))
...
def main() -> None:
    ...
    tem_ia = bool(
        ...
        or os.environ.get("LLM_PROVIDER", "").strip().lower() == "ollama"
    ) and not args.sem_ia
    if args.sem_ia:
    ...
    if not args.sem_monitoramento:
        console.print()
        _coletar_e_classificar_noticias(args.dias_monitoramento, tem_ia)
        _coletar_e_classificar_agenda(args.dias_monitoramento, tem_ia)

    linhas = montar_relatorio(args.mostrar_todas)
    noticias = montar_relatorio_noticias(args.mostrar_todas)
    eventos_agenda = montar_relatorio_agenda(args.mostrar_todas)

    if not linhas and not noticias and not eventos_agenda:
        console.print("Nada relevante encontrado no período (use --mostrar-todas para ver tudo).")
        return

    if linhas:
        imprimir_tabela(linhas)
    caminho_html = report.gerar_html(linhas, _RELATORIO_HTML, noticias=noticias, eventos_agenda=eventos_agenda)
