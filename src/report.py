...
_FONTE_NOTICIA_LABEL = {"anvisa": "🔵 Anvisa", "saude": "🟢 Ministério da Saúde", "ans": "🟣 ANS"}
...
_ESTILOS = """<style>
  ...
  * { box-sizing: border-box; }
  ...
  .monitor-banner { display: flex; align-items: center; gap: 1rem; text-decoration: none; color: inherit;
                      ...
                      margin-bottom: 1.6rem; box-shadow: var(--shadow); transition: transform .15s, border-color .15s; }
  .monitor-banner:hover { transform: translateY(-2px); border-color: var(--accent); }
  .monitor-banner-icon { font-size: 1.8rem; line-height: 1; flex: none; }
  .monitor-banner-texto { flex: 1; min-width: 0; }
  .monitor-banner-texto h3 { margin: 0 0 0.15rem; font-size: 0.98rem; }
  .monitor-banner-texto p { margin: 0; font-size: 0.8rem; color: var(--muted); }
  .monitor-banner-texto p span { color: var(--accent); font-weight: 700; }
  .monitor-banner-seta { font-size: 1.3rem; color: var(--accent); flex: none; transition: transform .15s; }
  .monitor-banner:hover .monitor-banner-seta { transform: translateX(3px); }

  .mon-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 0.7rem; }
  .mon-card { background: var(--card); border: 1px solid var(--border); border-radius: 14px;
               padding: 1rem 1.1rem; box-shadow: var(--shadow); transition: border-color .15s, transform .15s; }
  .mon-card:hover { border-color: var(--accent); transform: translateY(-1px); }
  .mon-card-topo { display: flex; align-items: center; gap: 0.5rem; font-size: 0.72rem; color: var(--muted);
                     margin-bottom: 0.5rem; flex-wrap: wrap; font-weight: 600; }
  .mon-card-topo .data { margin-left: auto; font-weight: 500; font-variant-numeric: tabular-nums; }
  .mon-card .titulo { display: block; font-weight: 700; font-size: 0.92rem; color: var(--text);
    margin-bottom: 0.35rem; line-height: 1.4; }
  .mon-card a.titulo { text-decoration: none;
                         margin-bottom: 0.4rem; line-height: 1.35; }
  .mon-card a.titulo:hover { text-decoration: underline; }
  .mon-card .local { font-size: 0.78rem; color: var(--muted); margin-bottom: 0.35rem; }
  .mon-card .justificativa { font-size: 0.8rem; color: var(--muted); margin: 0; line-height: 1.5; }
  .monitor-vazio { color: var(--muted); font-size: 0.85rem; padding: 0.5rem 0; grid-column: 1 / -1; }

  .mon-toolbar { display: flex; flex-wrap: wrap; gap: 0.6rem; align-items: center; margin-bottom: 1.1rem; }
  .mon-secao-grupo { margin-bottom: 2rem; }
  .mon-secao-grupo h2 { font-size: 0.95rem; margin: 0 0 0.9rem; display: flex; align-items: center; gap: 0.5rem; }
  .mon-secao-grupo h2 .contagem-mini { font-size: 0.76rem; color: var(--muted); font-weight: 500; }
...
{_MODAL_HTML}
  ...
  <footer>
    Dados públicos das APIs/serviços oficiais da Câmara dos Deputados, do Senado Federal, da Anvisa, do Ministério da Saúde, da ANS e do Planalto
    (Congresso dos EUA e Congresso Nacional do Chile também implementados, disponíveis via <code>--fontes</code>)
...
def gerar_html(linhas: list, caminho: str | Path, noticias: list | None = None,
    ...
    + agenda do Presidente — ver main.py). Só aparecem na página do Brasil
    (pais.html?p=BR), porque essas fontes são todas brasileiras — ver
    _gerar_pais_html()."""
...
def _gerar_pais_html(caminho: Path, gerado_em: str, versao: str) -> Path:
    ...
    <a class="monitor-banner rise" id="monitorBanner" href="noticias.html" hidden>
      <div class="monitor-banner-icon">📡</div>
      <div class="monitor-banner-texto">
        <h3>Monitoramento institucional</h3>
        <p>Notícias da Anvisa, Ministério da Saúde e ANS, mais a agenda pública do Presidente — <span id="monBannerContagem"></span></p>
      </div>
      <div class="monitor-banner-seta">→</div>
    </a>

    <div class="secao-titulo" id="tituloListaPls">Projetos de lei</div>
...
{_JS_ITEM_MODAL}
  ...
  // do Presidente — página própria, ver noticias.html/_gerar_noticias_html())
  // — só faz sentido pro Brasil, já que todas essas fontes são brasileiras.
  if (PAIS === 'BR') {{
...
def _gerar_noticias_html(caminho: Path, gerado_em: str, versao: str) -> Path:
    ...
    corpo_noticias = """    <div class="hub-hero rise" style="padding-top:0.5rem;">
      <h2>📡 Monitoramento institucional</h2>
      <p>Notícias da Anvisa, do Ministério da Saúde e da ANS, mais a agenda pública do Presidente —
      classificadas pelo mesmo critério de impacto na indústria farmacêutica usado pros PLs.</p>
    </div>

    <div class="kpis rise" id="kpis" style="animation-delay:.05s"></div>

    <div class="mon-toolbar rise" style="animation-delay:.1s">
      <div class="search">
        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="7"/><path d="m21 21-4.3-4.3"/></svg>
        <input type="text" id="busca" placeholder="Buscar por título, órgão, autoridade...">
      </div>
      <div class="segmented" id="fTipo" data-value="todos">
        <button data-v="todos" class="active">Tudo</button>
        <button data-v="noticia">Notícias</button>
      ...
      <select class="pill" id="fFonte">
        <option value="todos">Fonte: todas</option>
        <option value="anvisa">Anvisa</option>
        <option value="saude">Ministério da Saúde</option>
        <option value="ans">ANS</option>
        <option value="agenda">Agenda do Presidente</option>
      </select>
      <button class="clear-btn" id="limpar">Limpar</button>
    </div>

    <div class="mon-secao-grupo rise" style="animation-delay:.15rem">
      <h2>📰 Notícias <span class="contagem-mini" id="ctgNoticias"></span></h2>
      <div class="mon-grid" id="gridNoticias"></div>
    </div>
    <div class="mon-secao-grupo rise" style="animation-delay:.15rem">
      <h2>🎩 Agenda do Presidente <span class="contagem-mini" id="ctgAgenda"></span></h2>
      <div class="mon-grid" id="gridAgenda"></div>
    </div>"""

    script_noticias = f"""(function() {{
...
{_JS_HELPERS}
  ...
  function render() {{
    ...
    const noticiasVisiveis = tipo === 'agenda' ? [] : filtrar(NOTICIAS);
    const agendaVisivel = tipo === 'noticia' ? [] : filtrar(AGENDA);
  ...
  render();
    ...
    doc_noticias = _pagina(
        "Monitoramento — Radar Legislativo", "Anvisa · Ministério da Saúde · ANS · Presidência",
        topbar_noticias, corpo_noticias, script_noticias, versao=versao,
    )
