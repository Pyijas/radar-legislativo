"""Armazenamento local (SQLite) das proposições, notícias e eventos de
agenda coletados e classificados.

`proposicoes` cobre múltiplos países/casas legislativas (ver
src/paises.py) na mesma tabela. Cada proposição é identificada de forma
única por (pais, casa, id_externo) — não só por um id numérico — porque o
id de origem não é globalmente único (a Câmara e o Senado, por exemplo, têm
suas próprias sequências, e o Congress.gov dos EUA usa identificadores como
"hr1234", não numéricos).

`noticias` e `eventos_agenda` são tabelas separadas (não PLs, então não
cabem no schema de `proposicoes` — sem sigla_tipo/numero/ano, com campos
próprios como horário/local pra agenda) que seguem o mesmo padrão de
classificação (relevante/justificativa/áreas/tipo/nível), pra cobrir
notícias da Anvisa/Ministério da Saúde/ANS e a agenda pública de
autoridades (ver src/anvisa_news_client.py, src/saude_news_client.py,
src/ans_news_client.py, src/agenda_presidente_client.py) — adicionadas em
11/09/2026 a pedido do usuário, pra monitorar sinais de impacto na
indústria farmacêutica além dos PLs.
"""
...
def _connect() -> sqlite3.Connection:
    ...
    conn.execute(_SCHEMA_NOTICIAS)
    conn.execute(_SCHEMA_AGENDA)
    _migrar(conn)
    return conn


def _chave_noticia(fonte: str, id_externo: str) -> str:
    return f"{fonte}:{id_externo}"


def ja_processada_noticia(fonte: str, id_externo: str) -> bool:
    with _connect() as conn:
        cur = conn.execute(
            "SELECT 1 FROM noticias WHERE chave = ?", (_chave_noticia(fonte, id_externo),)
        )
        return cur.fetchone() is not None


def salvar_noticia(registro: dict) -> None:
    registro = dict(registro)
    registro["chave"] = _chave_noticia(registro["fonte"], registro["id_externo"])
    campos = [
        "chave", "fonte", "id_externo", "titulo", "resumo", "data_publicacao",
        "url_origem", "autor", "ia_disponivel", "fonte_classificacao", "relevante",
    ...
    with _connect() as conn:
        conn.execute(
            f"INSERT OR REPLACE INTO noticias ({colunas}) VALUES ({placeholders})", valores
        )


def listar_noticias_recentes(limite: int = 2000) -> list[sqlite3.Row]:
    with _connect() as conn:
        conn.row_factory = sqlite3.Row
        cur = conn.execute(
            "SELECT * FROM noticias ORDER BY data_publicacao DESC LIMIT ?", (limite,)
        )
        return cur.fetchall()


def _chave_agenda(autoridade: str, id_externo: str) -> str:
    return f"{autoridade}:{id_externo}"


def ja_processado_evento(autoridade: str, id_externo: str) -> bool:
    with _connect() as conn:
        cur = conn.execute(
            "SELECT 1 FROM eventos_agenda WHERE chave = ?", (_chave_agenda(autoridade, id_externo),)
        )
        return cur.fetchone() is not None


def salvar_evento_agenda(registro: dict) -> None:
    registro = dict(registro)
    ...
    with _connect() as conn:
        conn.execute(
            f"INSERT OR REPLACE INTO eventos_agenda ({colunas}) VALUES ({placeholders})", valores
        )


def listar_eventos_recentes(limite: int = 2000) -> list[sqlite3.Row]:
    with _connect() as conn:
        conn.row_factory = sqlite3.Row
        cur = conn.execute(
            "SELECT * FROM eventos_agenda ORDER BY data DESC, horario DESC LIMIT ?", (limite,)
        )
        return cur.fetchall()
