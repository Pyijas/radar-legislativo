...
def get_detalhe(item: dict) -> dict:
    ...
    if isinstance(detalhe, list):
        # Caso raro: projetos de congressos muito antigos (ex: 31º Congresso,
        # 1850) que só reaparecem na listagem porque a Library of Congress
        # atualizou metadados desta semana — não a apresentação original (ver
        # limitação no docstring do módulo). Pra esses, o endpoint de detalhe
        # devolve uma LISTA de versões duplicadas do mesmo número (numeração
        # de bill não era única na época) em vez de um único objeto.
        # Confirmado via teste direto em 04/09/2026 com HR1 do 31º Congresso.
        # Fica com a versão mais recente por updateDate; sem isso o resto da
        # função quebra com AttributeError (list não tem .get).
        detalhe = max(detalhe, key=lambda d: d.get("updateDate") or "", default={})
