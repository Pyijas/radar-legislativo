...
| País | Fonte | Formato | Nível de esforço | Status |
|---|---|---|---|---|
| 🇧🇷 Senado Federal | `legis.senado.leg.br/dadosabertos` | REST/JSON, oficial | Baixo | ✅ Implementado (`src/senado_client.py`) |
| 🇺🇸 EUA | `api.congress.gov` | REST/JSON, oficial, chave grátis | Baixo | ✅ Implementado (`src/congress_client.py`) — falta só cadastrar `CONGRESS_API_KEY` e rodar a primeira coleta |
| 🇨🇱 Chile | `tramitacion.senado.cl/wspublico` (achado na prática — cobre as duas casas, sem chave) | XML, oficial | Baixo, na prática | ✅ Implementado (`src/chile_client.py`) — ver nota abaixo |
...
**Nota sobre o Chile**: a pesquisa original abaixo apontava
`opendata.congreso.cl` + BCN (`datos.bcn.cl`, dados ligados/RDF) como o
caminho mais provável, com esforço "médio". Na implementação, encontrei em
`opendata.camara.cl` uma lista de web services do Senado
(`tramitacion.senado.cl/wspublico`) e um deles —
`tramitacion.php?fecha=DD/MM/YYYY` — devolve exatamente o que era preciso:
todos os projetos com movimento desde uma data, já com tramitação completa,
cobrindo as duas casas num único request, sem chave. Isso acabou sendo mais
simples que o caminho da BCN (RDF) cogitado originalmente — deixando a
seção "Chile" abaixo como registro histórico da pesquisa, não como o
caminho que foi seguido de fato. Limitação real descoberta na prática: o
serviço não aceita consultar mais de ~1 mês pra trás (sem backfill de
histórico grande, diferente de Brasil/EUA).
